...
export function loginUser(creds) {
    ...
    return (dispatch) => {
      ...
      if (!config.isBackend) { // 백엔드가 아니라면
        // console.log("Login Not Config is backend :: ",config, config.isBackend);
        dispatch(receiveToken('token'));
        dispatch(doInit());
        dispatch(push('/app'));
      } else { // 백엔드라면
        console.log("Login Not Config is backend :: ",config, config.isBackend);
        // 여기서 true가 나왔기 때문에 문제 없이 백엔드 부분이 실행이 되었는데 
        dispatch({
          type: LOGIN_REQUEST,
        });
        if (creds.social) {
          // console.log("Login creds.social :: ",creds, creds.social);

          window.location.href = config.baseURLApi + "/auth/signin/" + creds.social + '?app=' + config.redirectUrl;
        } else if (creds.email.length > 0 && creds.password.length > 0) {

          axios.post("/auth/signin/local", creds).then(res => { //로그인 정보를 api서버에 전달하고 creds,, 성공했을시 응답으로 받은 토큰을 처리하는 액션함수
            const token = res.data;
            dispatch(receiveToken(token)); //함수호출
            dispatch(doInit()); //
            dispatch(push('/app'));
          }).catch(err => {
            dispatch(authError(err.response.data)); 
          }) //axios를 통해서 api 요청을 보내고 요청이 실패할 경우 발생하는 에러를 처리하는 부분.
        } else {
