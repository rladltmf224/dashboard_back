    ...
    return (dispatch) => {
        ...
        if (!config.isBackend) {
            ...
            intervalId = setInterval(() => {
                console.log("droneper isBacend True -> ");
                    axios.get('/droneper').then(res => { //axios 라이브러리를 사용하여 http get요청을 보내고 서버로부터 데이터를 받아옴 / axios를 통해 /droneper api로 요청을 보냄
                    console.log("droneper res -> ", res.data);
