...
//   return (dispatch) => {
//     let intervalId;
//     console.log("droneper receiveDataRequest");
//     // We check if app runs with backend mode
//     if (!config.isBackend) {
//       console.log("droneper isBackend false -> ");
//       dispatch(receivingData());
//       new Promise((resolve) => {
//         resolve(mock.backendData); //resolve는 성공적으로 처리가 되었을때고 reject메서드는 말그대로 리젝 당했을 때 호출되는 거를 작성,
//       }).then((data) => {
//         //성공이 되었으니깐 then메서드가 호출되어 새로운 프로미스 객체가 반환된다.
//         dispatch(receiveDataSuccess(mock.backendData));
//         // console.log("droneper!!!!!!!!!!!!!!! -> ", mock.backendData);
//       });
//     } else {
//       dispatch(receivingData());
//       intervalId = setInterval(() => {
//         axios.get("/droneper").then((res) => {
//           //axios 라이브러리를 사용하여 http get요청을 보내고 서버로부터 데이터를 받아옴 / axios를 통해 /droneper api로 요청을 보냄
//           console.log("성공이니?", res.data);
...
export function receiveDataRequest() {
  return (dispatch) => {
    ...
    // We check if the app runs in backend mode
    if (!config.isBackend) {
      console.log("droneper isBackend false -> ");
      dispatch(receivingData());
      new Promise((resolve) => {
        ...
        .then((data) => {
          try {
            ...
            console.error("Error dispatching receiveDataSuccess:", error);
          }
        })
        .catch((error) => {
        ...
        axios
          ...
          .then((res) => {
            try {
              ...
              console.log("Error dispatching receiveDataSuccess:", error);
            }
          })
          .catch((error) => {
